...
export function UploadPanel({
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage(`Uploaded ${file.name}. Refresh to see the updated media list.`);
    form.reset();
    setSubmitting(false);
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
        ...
        <div className="mt-8">
          <h3 className="text-sm font-semibold text-white">Existing media</h3>
          <div className="mt-3 grid gap-3">
            {selectedRoom?.media?.length ? (
              selectedRoom.media.map((item) => (
                <div
                  key={item.id}
